param(
    [String]$Shell = "powershell",[switch]$Download=$false,[switch]$Verbose=$false,[string]$Architecture=64
    )
...
Write-Output "Getting public key"
Get-ChildItem "C:\temp\" | ForEach-Object { if ($_.Name -imatch ".*`.pub"){$key=$_} }

Write-Output "Setting content to administrators_authorized_keys file"
Get-Content "C:\temp\$key" | Out-File -Encoding utf8 "C:\ProgramData\ssh\administrators_authorized_keys" -Append
...
Write-Output "Changing sshd_config for using keys"
